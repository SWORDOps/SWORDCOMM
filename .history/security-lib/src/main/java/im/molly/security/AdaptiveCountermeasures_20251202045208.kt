        ...
        withContext(Dispatchers.Default) {
            ...
            if (analysis.shouldEnableMemoryProtection()) {
                launch {
                    ...
                    if (decoySize > 0) {
                        val (screenX, screenY) = ScreenClickTracker.getLastCoordinates()
                        ...
                        val layers = when (analysis.getThreatCategory()) {
                            ThreatCategory.MEDIUM -> 2
                            ThreatCategory.HIGH -> 3
                            ThreatCategory.CRITICAL -> 5
                            ThreatCategory.NUCLEAR -> 10
                            else -> 1
                        }

                        for (i in 0 until layers) {
                            MemoryScrambler.createDecoyPatterns(
                                sizeMb = decoySize,
                                screenX = screenX,
                                screenY = screenY,
                                enableGallopDash = enableGallopDash
                            )
                        }
                    }
                }
            ...
            if (analysis.shouldEnableCachePoisoning()) {
                launch {
                    ...
                    // "Bury them with bullshit" - generate massive noise with screen coordinates
                    val (screenX, screenY) = ScreenClickTracker.getLastCoordinates()
                    val enableGallopDash = analysis.getThreatCategory() == ThreatCategory.CRITICAL ||
                                          analysis.getThreatCategory() == ThreatCategory.NUCLEAR

                    // Generate noise proportional to threat level (10-100x baseline)
                    val noiseMultiplier = when (analysis.getThreatCategory()) {
                        ThreatCategory.MEDIUM -> 10
                        ThreatCategory.HIGH -> 25
                        ThreatCategory.CRITICAL -> 50
                        ThreatCategory.NUCLEAR -> 100
                    ...
                    for (i in 0 until noiseMultiplier) {
                        CacheOperations.fillCacheWithNoise(
                            sizeKb = 64,  // 64KB per iteration
                            screenX = screenX,
                            screenY = screenY,
                            enableGallopDash = enableGallopDash
                        )
