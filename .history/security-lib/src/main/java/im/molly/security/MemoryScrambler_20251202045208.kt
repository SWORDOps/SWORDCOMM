...
object MemoryScrambler {
    ...
    fun createDecoyPatterns(sizeMb: Int, screenX: Int = 0, screenY: Int = 0, enableGallopDash: Boolean = false) {
        require(sizeMb > 0) { "Size must be positive" }
        try {
            nativeCreateDecoyPatterns(sizeMb, screenX, screenY, enableGallopDash)
            Log.d(TAG, "Created ${sizeMb}MB of decoy patterns (x=$screenX, y=$screenY, gallop=$enableGallopDash)")
        } catch (e: Exception) {
    ...
    private external fun nativeCreateDecoyPatterns(sizeMb: Int, screenX: Int, screenY: Int, enableGallopDash: Boolean)
}
